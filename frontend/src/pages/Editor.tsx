...
import { fetchProjectById, fetchScenes, createScene, updateScene, setCurrentProject } from '../store/projectSlice';
...
const Editor = () => {
  ...
  const handleCreateScene = async () => {
    ...
    const characters = newSceneCharacters.split(',').map(c => c.trim()).filter(Boolean);
    await dispatch(createScene({
      ...
      sceneData: {
        ...
        characters,
        content: `【${newSceneLocation === 'interior' ? '内景' : '外景'} - ${newSceneTitle}】\n\n${newSceneTime === 'day' ? '白天。' : newSceneTime === 'night' ? '夜晚。' : '黄昏。'}`,
    ...
    <div className="flex h-screen">
      <div className={`flex flex-col w-64 bg-midnightLight border-r border-slate/30 transition-all duration-300 ${showSceneList ? '' : 'w-0 overflow-hidden'}`}>
        <div className="p-4 border-b border-slate/30">
          ...
          <p className="text-xs text-textMuted">{template.name}</p>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          <div className="space-y-2">
            {template.acts.map((act, actIndex) => (
              <div key={actIndex}>
                <button
                  className="w-full flex items-center justify-between p-2 text-textSecondary hover:text-amber transition-colors"
                >
      ...
      <div className="flex-1 flex flex-col">
        <div className="flex items-center justify-between p-4 bg-midnightLight border-b border-slate/30">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={handleSaveScene} className="btn-secondary flex items-center gap-2">
            ...
            >
              <div className="p-4">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="font-medium text-textPrimary">AI辅助创作</span>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-4 bg-midnightLight rounded-lg border border-slate/30">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className="text-sm font-medium text-textPrimary">对话润色</span>
                    </div>
                    <p className="text-xs text-textSecondary mb-3">选中台词后点击下方按钮进行润色</p>
                    <div className="flex flex-wrap gap-2">
                      {dialogStyles.map((style) => (
                        <button
                          ...
                          className="px-3 py-1.5 bg-midnight border border-slate rounded-lg text-xs text-textSecondary hover:border-amber hover:text-amber transition-all"
                        >
                  ...
                  <div className="p-4 bg-midnightLight rounded-lg border border-slate/30">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className="text-sm font-medium text-textPrimary">氛围描摹</span>
                    </div>
                    <p className="text-xs text-textSecondary mb-3">选择场景关键词生成环境描写</p>
                    <div className="flex flex-wrap gap-2">
                      {atmosphereKeywords.map((keyword) => (
                        <button
                          ...
                          className="px-3 py-1.5 bg-midnight border border-slate rounded-lg text-xs text-textSecondary hover:border-purple hover:text-purple transition-all"
                        >
                          {keyword}
                        </button>
                      ))}
            ...
            {selectedScene ? (
                ...
                <div className="flex items-center gap-4 mb-6">
                  <div className="flex items-center gap-2">
                    <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                      selectedScene.location === 'interior' ? 'bg-forest/20 text-forestLight' : 'bg-purple/20 text-purpleLight'
                    }`}>
                  ...
                  <div className="flex items-center gap-2 text-textSecondary">
                    ...
                    <span className="text-sm">{selectedScene.characters.join(', ')}</span>
                  </div>
                </div>
              ...
              <div className="h-full flex items-center justify-center">
                ...
                >
                  ...
                  <h3 className="text-lg font-display text-textPrimary mb-2">选择或创建场景</h3>
                  <p className="text-textSecondary text-sm">从左侧列表选择场景开始编辑，或点击「新建场景」创建新的场次</p>
                </motion.div>
              </div>
            )}
          </div>
          ...
          <div className="w-72 bg-midnightLight border-l border-slate/30 p-4 overflow-y-auto">
            ...
            {selectedScene && (
              <div className="space-y-4">
                <div>
                  <label className="block text-textSecondary text-xs mb-1">场景标题</label>
                  <input
                    type="text"
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="block text-textSecondary text-xs mb-1">场景类型</label>
                  ...
                  <div>
                    <label className="block text-textSecondary text-xs mb-1">时间</label>
                    <div className="p-2 rounded-lg text-xs text-center bg-midnight text-textSecondary">
                      {selectedScene.timeOfDay === 'day' ? '白天' : selectedScene.timeOfDay === 'night' ? '夜晚' : '黄昏'}
                    </div>
                  </div>
                </div>
                <div>
                  <label className="block text-textSecondary text-xs mb-1">出场角色</label>
                  <div className="flex flex-wrap gap-1">
                    {selectedScene.characters.map((char) => (
                      <span key={char} className="px-2 py-1 bg-midnight rounded text-xs text-textSecondary">
                        {char}
                      </span>
                    ))}
                  </div>
                ...
                <div>
                  <label className="block text-textSecondary text-xs mb-1">字数统计</label>
                  <div className="p-3 bg-midnight rounded-lg">
                    <p className="text-2xl font-display text-amber">{sceneContent.length}</p>
                    <p className="text-xs text-textMuted">字符</p>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="mt-8">
              <h3 className="font-display text-textPrimary mb-4">卡文救援</h3>
                ...
                {['意外干扰', '情感爆发', '秘密揭露'].map((type, index) => (
                  <motion.button
                    ...
                    onClick={() => {
                      ...
                      api.ai.getPlotSuggestions(currentProject.id).then((suggestions) => {
      ...
      {showCreateSceneModal && (
        ...
        >
          ...
          >
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-textSecondary text-sm mb-2">场景标题</label>
                <input
                  type="text"
                  value={newSceneTitle}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-textSecondary text-sm mb-2">场景类型</label>
                  <div className="flex gap-2">
                    <button
                      onClick={() => setNewSceneLocation('interior')}
                      className={`flex-1 py-2 rounded-lg text-sm ${
                        newSceneLocation === 'interior' ? 'bg-forest text-textPrimary' : 'bg-midnight border border-slate text-textSecondary'
                      }`}
                    >
                      内景
                    </button>
                    <button
                      onClick={() => setNewSceneLocation('exterior')}
                      className={`flex-1 py-2 rounded-lg text-sm ${
                        newSceneLocation === 'exterior' ? 'bg-purple text-textPrimary' : 'bg-midnight border border-slate text-textSecondary'
                      }`}
                    >
                      外景
                    </button>
                ...
                <div>
                  <label className="block text-textSecondary text-sm mb-2">时间</label>
                  <div className="flex gap-2">
                    <button
                      onClick={() => setNewSceneTime('day')}
                      className={`flex-1 py-2 rounded-lg text-sm ${
                        newSceneTime === 'day' ? 'bg-amber text-midnight' : 'bg-midnight border border-slate text-textSecondary'
                      }`}
                    >
                      <Sun className="w-4 h-4 mx-auto" />
                    </button>
                    <button
                      onClick={() => setNewSceneTime('night')}
                      className={`flex-1 py-2 rounded-lg text-sm ${
                        newSceneTime === 'night' ? 'bg-purple text-textPrimary' : 'bg-midnight border border-slate text-textSecondary'
                      }`}
                    >
              ...
              <div>
                <label className="block text-textSecondary text-sm mb-2">出场角色（用逗号分隔）</label>
                <input
                  type="text"
      ...
      {(showPolishModal || showAtmosphereModal) && (
        ...
        >
          ...
          >
            <h2 className="text-xl font-display text-textPrimary mb-4">
              {showPolishModal ? 'AI对话润色结果' : 'AI氛围描摹结果'}
            </h2>
